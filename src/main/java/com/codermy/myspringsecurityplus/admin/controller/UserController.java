...
public class UserController {
    ...
    @PreAuthorize("hasAnyAuthority('user:list')")
    @MyLog("查询用户")
    public Result<SysUser> userList(PageTableRequest pageTableRequest, SysUser sysUser){
        pageTableRequest.countOffset();
        return userService.getAllUsersByPage(pageTableRequest.getOffset(),pageTableRequest.getLimit(), sysUser);
    }
    ...
    public Result<SysUser> saveUser(@RequestBody SysUser sysUser){
        ...
        if (UserConstants.USER_NAME_NOT_UNIQUE.equals(userService.checkUserNameUnique(sysUser))){
            return Result.error().message("用户名已存在");
        }
        BCryptPasswordEncoder bCryptPasswordEncoder = new BCryptPasswordEncoder();
        sysUser.setPassword(bCryptPasswordEncoder.encode("123456"));
        return userService.save(sysUser, sysUser.getRoleId());
    }
    ...
    public Result updateUser(@RequestBody SysUser sysUser){
        SysUser userById = userService.getUserById(sysUser.getUserId());
        ...
        return userService.updateUser(sysUser, sysUser.getRoleId());
    }
    /**
     * 用户状态修改
     */
    ...
    public Result changeStatus(@RequestBody SysUser sysUser)
    {
        ...
        userService.changeStatus(sysUser);
        return Result.judge(userService.changeStatus(sysUser),"修改成功");
    }
